...
-(void)addProductAtMarketWhithNameProduct:(int)nameProduct AndIdMercado:(int)idMercado AndValueProduct:(int)subTotal AndCantProduct:(int)cantProduct
{
    [self searchPathOfDatabase];
    sqlite3_stmt * queryInsert;
    
    if (sqlite3_open(dbPath, &conexDB)==SQLITE_OK) {
        NSString * stringInsert = [NSString stringWithFormat:@"INSERT INTO tbl_mercado (idmercado, idproducto, valor_sub,cantidad) VALUES (%i,%i,%i,%i)", idMercado,nameProduct, subTotal,cantProduct];
